...
class ScaffoldMolSampler(data.Sampler):
    ...
    def __init__(self, dataset, batch_size, num_iterations, exclude_ids_loc=None, training=None, split_type=None):
        """Building the sampler

        Args:
            dataset (Dataset): The dataset to be sampled
            batch_size (tuple[int, int]): The batch sizes
            exclude_ids_loc (str or None): The location storing the ids to be excluded from the training set
            split_type (str or None): The type of splits
            training (bool or None): Whether the sampler is used for training data or for test data
            num_iterations (int): The number of total iterations
        """
    ...
    def __iter__(self):
        ...
        """
        shuffled_index_scaffold = []
        shuffled_index_molecule = []

        for _ in range(self.num_iterations):  # TODO: split. sample by scaffold and sample by molecules
            # Sample by scaffold
            batch_scaffold = []
