...
class MoleculeSpec:
    ...
    def __init__(self, file_name=os.path.join(os.path.dirname(__file__), 'atom_types.txt'),
                 max_num_atoms=50, max_num_bonds=100):
        """The helper class to store information about atom types and bond types

        Args:
            file_name: The location where atom type information and bond information is stored,
                       default to 'datasets/atom_types.txt'
            max_num_atoms: The maximum allowed size of molecules measured using the number of heavy atoms, default to 50
            max_num_bonds: The maximum allowed size of molecules measured using the number of bonds, default to 100
        """
        self.atom_types = []
        self.atom_symbols = []
        ...
        self.bond_orders = [Chem.BondType.AROMATIC, Chem.BondType.SINGLE, Chem.BondType.DOUBLE, Chem.BondType.TRIPLE]
    ...
    def index_to_atom(self, idx):
        ...
        Args:
            idx (int): The type of atom to create

        Returns:
            Chem.Atom: The new atom created
        """
    ...
    def index_to_bond(self, mol, begin_id, end_id, idx):
        ...
        Args:
            mol (Chem.Mol): The molecule where the new bond should be created
            begin_id, end_id (int): The (starting and terminating) location of the new bond
            idx (int): The type of the new bond
        """
